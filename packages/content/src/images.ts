 ...
 */
const SOURCE =
  (typeof process !== "undefined" && process.env?.SEED_IMAGE_SOURCE) || "picsum";
 ...
 */
function blurSvg(seed: string): string {
...
function localSvg(seed: string, w: number, h: number, label: string): string {
  const pair = seededPick(seed, DUOTONE_PAIRS);
 ...
 */
export function seedImage(seed: string, opts: SeedImageOptions = {}): Image {
