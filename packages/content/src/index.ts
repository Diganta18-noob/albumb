import type { Chapter, Category, LocationPoint } from "@chronicles/types";
...
export function mapLocations(): LocationPoint[] {
  return memories
    ...
    .map((m) => ({
      ...
      lat: m.location.lat as number,
      lng: m.location.lng as number,
      kind: locationKind(m.category),
      memoryIds: [m.id],
      createdAt,
    }));
}

/** Map a memory category onto the map's smaller pin vocabulary. */
function locationKind(category: string): LocationPoint["kind"] {
  switch (category) {
    case "trips":
      return "trip";
    case "office":
